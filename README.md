...
`extract-files.sh` produces the untracked `vendor/onyx/leaf3` tree. It retains
a conservative set of device-specific Qualcomm/ONYX blobs rather than copying
the complete stock vendor partition, which also contains files generated by
the Lineage build. Do not commit or redistribute those proprietary files.
